 ...
 *
 * Responsibilities:
 * - Intent Detection via Gemini (GREETING / CLARIFY / SEARCH / TRYON_SPECIFIC)
 * - GREETING → Warm welcome with capability overview
 * - CLARIFY → Follow-up questions about style preferences
 * - SEARCH → Generate search query via Gemini, then call live web scraper
 * - TRYON_SPECIFIC → Prepare state for the Artist agent
 * - Gender-aware query construction
 */
...
Reply with ONLY the intent word, nothing else.`;
    ...
    const intentVal = intentResult.response.text().toUpperCase().trim();
    console.log(`[Stylist] Intent detected: ${intentVal}`);
...
Respond with a brief, enthusiastic confirmation (1-2 sentences) that you are initiating the virtual try-on for this piece. Use an emoji.`;
        ...
        const title =
          state.garmentTitle ||
          latestQuery.replace("I want to try on this item: ", "").trim() ||
          "Selected Item";
        ...
        state.stylingAdvice =
          "I'd love to help you try that on, but I don't have the image for it. Could you share a product link or upload a garment photo?";
      }
    }
    ...
    // ── 5. SEARCH ────────────────────────────────────────
      ...
      console.log("[Stylist] Preparing live web search...");

      // Gender-aware query generation
      const gender = state.userGender || "";
      let genderHint = "";
      if (gender === "male")
        genderHint =
          "The user is male. Prefix the query with 'mens' if not already specified.";
      else if (gender === "female")
        genderHint =
          "The user is female. Prefix the query with 'womens' if not already specified.";
...
Generate a 3-5 word search query to find the exact clothing item the user wants on an e-commerce site (e.g. 'mens blue linen shirt'). Reply ONLY with the query, no quotes.`;
      ...
      const searchString = queryResult.response
        .text()
        .trim()
        .replace(/['"]/g, "");
