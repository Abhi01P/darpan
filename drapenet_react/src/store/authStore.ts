...
export const useAuthStore = create<AuthState>((set) => ({
  ...
  initialize: async () => {
    ...
    try {
      ...
      set({ user: { email: res.data.email, name: res.data.name, gender: res.data.gender || null, digital_twin: res.data.digital_twin }, isLoading: false });
    } catch {
  ...
  login: async (email, password) => {
    ...
    try {
      ...
      set({ user: { email: meRes.data.email, name: meRes.data.name, gender: meRes.data.gender || null, digital_twin: meRes.data.digital_twin }, isLoading: false });
      return true;
    } catch (err: unknown) {
  ...
  updateAvatar: async (url: string | null) => {
    try {
      const res = await api.put('/auth/me/avatar', { avatar_url: url });
      set({ user: { email: res.data.email, name: res.data.name, gender: res.data.gender || null, digital_twin: res.data.digital_twin }});
    } catch (err) {
